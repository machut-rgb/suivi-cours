...
const getMonthlyProgression = (chapitres) => {
    ...
    chapitres.forEach((chapitre) => {
        const date = new Date(chapitre.updated_at);
        ...
        if (chapitre.isFinished) {
            monthlyProgress[monthKey].completed = prev + 1;
            prev++;
        }
    });
    ...
    return Object.entries(monthlyProgress).map(([month, data]) => ({
        month,
        progression: ((data.completed / data.total) * 100).toFixed(2),
    }));
};
...
export default function ResponsableDashboard({ title, programs }) {
    ...
    const stats = {
        ...
        totalMatieres: new Set(programs.map((p) => p.matiere.classe)).size,
    };
                    ...
                    {[
                        ...
                        >
                            <div className="flex items-center">
                                <div
                                    className={`mr-3 flex h-8 w-8 items-center justify-center rounded-full bg-${stat.color}-200 dark:bg-${stat.color}-900`}
                                >
